...
### 3.2 分叉闭环
- 回复气泡动作和选中文案悬浮动作均先进入“引用态”（不立即建分叉）。
- 用户发送下一条问题后，才创建新的分叉 Q-A Block。
- 首次分叉发送后切换到图谱模式。
- 图谱节点按“一问一答 Pair”组织，节点内容动态高度展示（不截断）。
- 图谱模式中仅保留图谱与底部输入条，不展示完整聊天历史面板。
...
## 5.3 分叉
1. 用户在消息气泡或拓扑 Block 中触发分叉后，系统先把引用内容放入输入区上方。
2. 用户发送下一条追问时，系统基于引用来源创建新 Q-A Block 并连边。
3. 选区分叉使用选中文案作为引用；非选区分叉使用来源消息前 100 字作为引用预览。
4. 若会话存在任意分叉，主区渲染图谱 + 底部输入条。
...
## 8. 风险与缓解
- 风险: 首次分叉前后（线性 -> 拓扑）的状态迁移可能导致节点映射错误。
  - 缓解: 线性消息先规范化为一问一答链，再按来源消息映射父节点创建分叉块。
