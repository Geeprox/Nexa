...
- A1 聊天发送 + Mock 流式回复。
- A2 Retry 版本链与版本切换控制。
- A3 气泡分叉与选区分叉统一改为“先引用态，后发送建分叉”。
- A4 首次分叉发送后切换图谱，拓扑态仅保留图谱 + 输入条。
- A5 拓扑节点重构为“一问一答 Pair”（动态高度，不限高）。
...
## 3. 当前状态快照
- lint: 通过。
- test: 24 files / 94 tests 全通过。
- coverage: Statements 88.01%, Branches 78.71%, Functions 85.53%, Lines 88.01%。

## 4. 发布阻断项（Gate）
- 任一关键入口不可点击。
- 图谱模式失去输入能力。
- 分叉触发时未进入引用态，或未在发送后建块。
- 拓扑态仍展示线性聊天历史面板。
- 会话状态刷新丢失或持久化损坏。
- 设置保存失败且无可见反馈。
